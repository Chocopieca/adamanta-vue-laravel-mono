...
Vue.mixin({
  ...
  methods: {
    ...
    getTitle() {
      return {
        category: (arr, link) => this.getCategoryTitle(arr,link),
        product: (arr, link) => this.getProductTitle(arr,link),
      }
    },
    getCategoryTitle(arr, link) {
      let title = '';
      arr.forEach(item => {
        if(link === item.link) {
          title = item.title
    ...
    getProductTitle(arr, link) {
      let title = '';
      arr.forEach(item => {
        if(link === item.linkGroup.product) {
          title = item.title
        }
      })
      return title;
